...
int main(int argc, char *argv[])
{
  ...
  if((se=(uint8_t*)malloc(sizeX*sizeY*sizeof(uint8_t))) == NULL) perror("Malloc");
  readPGM(se,"img/ball.pgm", &sizeX, &sizeY);
  /* for (i=0; i<sizeX*sizeY; i++) if (se[i]!=0) se[i]=1; */
  posX = sizeX/2;
  posY = sizeY/2;
  
  for (i=0; i<x*y; i++) imageIn2[i]=imageIn[i];

  printf("Running dilation_arbitrary_SF ... (8 bits/grayscale only!)");  
  dilation_arbitrary_SF(imageIn2, imageOut2, x, y, se, sizeX, sizeY, posX, posY);
  printf(" finished.\n");

  for (i=0; i<x*y; i++) 
  	{
	if (imageOut2[i]<0) imageOut2[i]=0;
	if (imageOut2[i]>255) imageOut2[i]=255;
	imageOut[i]=imageOut2[i];
	}

  writePGM(imageOut, x, y, 0, 0, "dilation_arbitrary");
